...
Module ModeList
    ...
    Public ReadOnly AliasModes As New Dictionary(Of String, String)() From {
        {"A3J", "SSB"}, {"LSB", "SSB"}, {"USB", "SSB"}, {"A1", "CW"}, {"A1A", "CW"}, {"F3", "FM"},
    ...
    Public ReadOnly MisReadingModes As New Dictionary(Of String, String)() From {
        {"FTS", "FT8"}, {"FTZ", "FT8"}, {"FI8", "FT8"}, {"F18", "FT8"}, {"EI8", "FT8"}, {"FTJ", "FT8"},
...
Module CalendarModule                ' 月名 → 数字 の辞書
    ...
    Public ReadOnly MisReadingMonth As New Dictionary(Of String, String)() From {
        {"3AN", "JAN"}, {"APE", "APR"}, {"/UL", "JUL"}, {"1UL", "JUL"}, {"0CT", "OCT"}, {"N0V", "NOV"}
    }
...
Partial Class frmMain
    ...
    Private Shared ModeTime As Detection                   ' Timmeに一致するものが2個以上あったとき
    ...
    Public Class RegexExtractor
        ...
        Public Shared Function ExtractCallsignCandidates(text As String) As List(Of String)
            ...
            ' "7 K 2 XRZ "のように空白が入っている場合の処理
            'Dim callPattern As String = "^([A-Z]{1,2}|[1-9][A-Z]|[A-Z]\d|3DA)\d[A-Z]{1,3}$"
            'Dim cs As String
            ''   text = text.Replace(" ", "")
            'For Each m As Match In Regex.Matches(text, pattern)
            '    Dim fixedCs = m.Value
        ...
        Public Shared Function ExtractDate(text As String) As String
            ...
            If m.Success = True Then
                ...
                SetDateBaseDetection("DATE", m.Index, m.Length, row, col)
            End If
            ...
            For Each key In MonthNames.Keys
                ...
                For Each m1 As Match In ms
                    Dim d As Double = Similarity(key, m1.Value)
                    If d > 0.74 Then
                        text = text.Replace(m1.Value, key)
                        If IsUsed(m1.Index, m1.Length) Then
                            dateFromMonth = ExtractDateWithMonthName(text)
                            If dateFromMonth <> "" Then
                                UsedRanges.Add((m1.Index, m1.Length))
        ...
        Private Shared Function ExtractDateWithMonthName(text As String) As String
            ...
            For Each key In MonthNames.Keys
                pattern = "\b(\')?(\d{1,4})[\s/-]*" & key & "[\s/-]*(\')?(\d{1,2})\b"              '年、月、日または日、月、年の並び順
                ...
                If m.Success Then
                    ...
                    Else
                        ...
                        If year.Length = 3 AndAlso DateFormat = DateFormats.IsoFormat Then
        ...
        Private Shared Function ExtractDateNumeric(text As String) As String
            ...
            ' 年が真ん中に来るはずない
            Dim pattern = "(^|\s|\D|:|;)(')?(\d{2}|\d{4}|\d)[/\-\.\s](\d{1,2})[/\-\.\s](')?(\d{2}|\d{4}|\d)(\s|\D|$)"
            Dim ms = Regex.Matches(text, pattern)
            ...
            For Each m As Match In ms
                ...
                Dim part2 = m.Groups(4).Value
                Dim part3 = m.Groups(6).Value
                Dim year As String = ""
                Dim month As String = ""
                Dim day As String = ""

                If (m.Groups(2).Value = "'") OrElse (part1.Length = 4) OrElse (DateFormat = DateFormats.IsoFormat) Then         ' (yy)yy-mm-dd  Japanese Format の場合
                    year = part1
                    month = part2
                    day = part3
                ElseIf (DateFormat = DateFormats.AmericanFormat) Then     ' mm-dd-(yy)yy American Format の場合
                    day = part2
                    month = part1
                    year = part3
                ElseIf (m.Groups(5).Value = "'") OrElse (part3.Length = 4) _
                    OrElse (DateFormat = DateFormats.BritishFormat) OrElse (DateFormat = DateFormats.AmericanFormat) Then     ' dd-mm-(yy)yy British Format の場合
                    day = part1
                    month = part2
                    year = part3
                Else
                ...
                If m.Index > DateBase.Index Then
        ...
        Public Shared Function ExtractTime(text As String) As String
            ...
            ' 時刻パターン（例：09:00JST など）
            pattern = "\s([012]\d)[:\./\s-]([0-5]\d)[\s]*([UJZ729])"
            ...
            ' 時刻パターン（例：TIME: 09:00JST など）7と2は誤認識の可能性がので、JSTの代わりにJ72も許容する
            pattern = "T[IME1]{3}[ :;]*([012]\d)[:\./\s-]*([0-5]\d)[\s]*[UJZ72\D]"
            m = Regex.Match(cleaned, pattern, RegexOptions.IgnoreCase)
            If m.Success Then
                Dim hour = Integer.Parse(m.Groups(1).Value)
                Dim minute = Integer.Parse(m.Groups(2).Value)
            ...
            ' 時刻パターン（例：TIME: 09000JST など）":"が"1"と誤読した場合、
            pattern = "T[IME1]{3}[ :;]*([0-2]\d.?[0-5]\d)[\s]*[UJZ729]"
            m = Regex.Match(cleaned, pattern, RegexOptions.IgnoreCase)
            If m.Success Then
                Dim hour = Integer.Parse(Strings.Left(m.Groups(1).Value, 2))
                Dim minute = Integer.Parse(Strings.Right(m.Groups(1).Value, 2))
            ...
            ' 時刻パターン（例：9:3, 09:30, 21:5 など）両端に空白がある場合のみ抽出する
            pattern = "(?<=\s)([012]\d)[:\./s]([0-5]\d)([\s]*[UJZ72]*(?=\s))"
            ...
            ' 時刻パターン hhnn(2300)　両端に空白がある場合のみ抽出する 時と分の間に区切りがない
            '            pattern = "(?<=\s)([012]\d)[/s:;]*([0-5]\d)([\s]*[UJZ72]*(?=\s))"
            pattern = "([\s])([01]\d|2[0-3])[:;\s]*([0-5]\d)([UJZ72\D\s$])"
            'pattern = "(?<!\d)([01]\d|2[0-3])[:;\s]*([0-5]\d)([UJZ]\b|\s|$)"

            Dim ms = Regex.Matches(cleaned, pattern, RegexOptions.IgnoreCase)
            For Each m In ms
                Dim hour = Integer.Parse(m.Groups(2).Value)
                Dim minute = Integer.Parse(m.Groups(3).Value)
                ' 時刻の妥当性チェック
                If hour < 0 Or hour > 23 Then Return ""
                If minute < 0 Or minute > 59 Then Return ""
                If Not IsUsed(m.Index, m.Length) Then
        ...
        Public Shared Function ExtractMode(text As String) As String
            ...
            If m.Success = True Then
                ...
                SetBandBase("MODE", m.Groups(1).Index, m.Groups(1).Length, row, col)        ' 周波数が複数見つかったときのため 見つかったmodeを基準にする
            End If
            ...
            For Each key In Modes
                ...
                If m.Success = True Then
                    ...
                    AddModeBase(key, m.Groups(1).Index, m.Groups(1).Length, 0, 0)

                    '                GetModeBase(cleaned, m)
                End If
            Next

            For Each f In DetectionModes
                With ModeBase
                    ...
                    If s < shortestDistance Then
                        ...
                        shortestDistance = s
                        SetModeBase(f.Value, f.Index, f.Length, f.Row, f.Column)        ' Band選択のため
            ...
            For Each keys In AliasModes
                ...
                If m.Success = True Then
                    ...
                    UsedRanges.Add((Index, Length))                     ' 二重かっこは配列化のため
                    Dim row = text.LastIndexOf(ControlChars.Lf, Index)
                    Dim column = Index - text.LastIndexOf(ControlChars.Lf, Index)
                    AddModeBase(keys.Value, Index, Length, row, column)

                    SetModeBase(m.Groups(1).Value, Index, Length, row, column)        ' Band選択のため
            ...
            For Each keys In MisReadingModes
                ...
                If m.Success = True Then
                    ...
                    Dim row = text.LastIndexOf(ControlChars.Lf, index)
                    Dim column = index - text.LastIndexOf(ControlChars.Lf, index)
                    AddBandBase(keys.Value, index, length, row, column)
                ...
                For Each m In md
                    ...
                    If d > 0.74 Then
                        ...
                        If idx >= 0 Then
                            ...
                            Dim row = text.LastIndexOf(ControlChars.Lf, Index)
                            Dim column = Index - text.LastIndexOf(ControlChars.Lf, Index)
                            AddBandBase(key, Index, length, row, column)
            ...
            For Each key In Modes           ' Modesの中で、誤認識されやすいものを抽出する
                ...
                If m.Success = True Then
                    ...
                    SetModeBase(m.Groups(2).Value, Index, Length, row, column)        ' Band選択のため

                    Return key
                    '    UsedRanges.Add((m.Index, m.Length))     '
                    'AddBandBase(key, m.Index, m.Length, 0, 0)   ' とりあえず 
                    'Return key
                End If
            Next
        ...
        Private Shared Function ReportToMode(Band As String) As String
            If isPhone = True Then

                Dim mode As String = "SSB"
                For i = 0 To BandList.Bands.Length - 1
                    If Band = BandList.Bands(i) Then
                        Return mode
                    End If
                    If BandList.Bands(i) = "2M" Then
                        Return "FM"
                    End If
                Next
            End If
        ...
        Public Shared Function ExtractBand(text As String) As String
            ...
            Else
                BandBase = ModeBase     ' BandBaseが見つからないときは、Modebaseで代用する
            End If
        ...
        Private Shared Function ExtractFrequency(text As String) As String
            ...
            '           pattern = "(?<=\s|:|;|\b)(\d{1,7})([\s\.,]){0,2}(\d{0,3})?([M|K|G|\s|])"           ' 直前が空白であり、直後が数字以外であること
            pattern = "(?<=\s|:|;|\b)(\d{1,7})([\s\.,]){0,2}?(\d{0,3})?(?=[\s\]MKG]|$)"           ' 直前が空白であり、直後が数字以外であること
            ...
            For Each m In Regex.Matches(text, pattern)
                ...
                ElseIf unit = "GHZ" Then                                    ' GHzで記入されている場合 → MHzに変換する
                    ...
                    Else
                        Continue For
                    End If
                ElseIf (decimalPart <> "0") AndAlso (integerPart.Length <> 0) Then         ' 7.025 14.200等
                    ...
                    End If
                ElseIf (decimalPart = "0") AndAlso (integerPart.Length > 3) Then       ' 7025 14200等
            ...
            Debug.Print(BandBase.Column & " " & BandBase.Row)
            If ModeBase.Index <> 0 Then
                BandBase = ModeBase
            End If

            For Each f In DetectionBands
                With BandBase
                    'Debug.Print("BandBase: " & .Value & " at (" & .Index & "," & .Length & ") - Candidate: " & f.Value & " at (" & f.Index & "," & f.Length & ")")
                    'Dim s As Integer = (Math.Abs(.Column - f.Column) ^ 2 + Math.Abs(.Row - f.Row) * 1.3) ^ 2
                    'Dim s As Integer = Math.Min(Math.Abs(.Index - f.Index), Math.Abs(.Index + .Length - f.Index + f.Length))
                    Dim s As Integer = Math.Abs(.Index - f.Index)
                    '      If (f.Index > .Index) AndAlso (s < shortestDistance) Then
                    If (s < shortestDistance) Then
                        bestBand = f.Value
                        IndexOfBestFreq = f.Index
                        lengthOfBestFreq = f.Length
                        shortestDistance = s
                    End If
                End With
            Next
        ...
        Public Shared Sub ClearBaseDetection(Base As Detection)
            With Base
                .Value = ""
                .Index = 0
                .Length = 0
        ...
        Public Shared Sub AddBaseDetection(Base As Detection, val As String, idx As Integer, Len As Integer, row As Integer, column As Integer)
            With Base
                .Value = val
                .Index = idx
                .Length = Len
